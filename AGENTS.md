...
4. **diff 审核**：任何 `git commit` 前，必须先完整检查 `git diff`；默认向用户或主审核 AI 展示 diff 并等待其明确批准。仅当当前任务包明确授权该任务自行检查完整 diff 后提交时，才可按该授权提交。
5. **push 审核**：任何 `git push` 都必须获得用户明确同意，不因改动简单或明显而跳过；任务内的 commit 授权不等于 push 授权。
