...
6. **PR 描述与版本追溯**：创建或更新 PR 必须完整使用 `.github/pull_request_template.md`，如实填写全部章节，不得保留占位符或以临时摘要替代。PR 必须填写目标开发版本、关联开发计划、关联任务或问题；修复类 PR 还必须填写修复归属版本和所修复问题。开发计划版本用于追溯改动归属，必须与 `pyproject.toml`、`CHANGELOG.md` 和 tag 所表示的项目正式发布版本分开说明。
