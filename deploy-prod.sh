...
ssh -o BatchMode=yes -o ConnectTimeout=8 "${REMOTE_HOST}" \
  "cd '${REMOTE_DIR}' && mkdir -p '${REMOTE_DOCKER_CONFIG}' && DOCKER_CONFIG='${REMOTE_DOCKER_CONFIG}' docker compose build claw-news"
...
echo "[4/4] 健康检查"
ssh -o BatchMode=yes -o ConnectTimeout=8 "${REMOTE_HOST}" \
  "cd '${REMOTE_DIR}' && curl -fsS --max-time 10 '${HEALTH_URL}'"
