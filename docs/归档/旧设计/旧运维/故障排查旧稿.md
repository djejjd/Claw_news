...
```bash
cat ${CLAW_NEWS_DIR}/data/publish_status.json | python3 -m json.tool
```
...
```bash
wc -l ${CLAW_NEWS_DIR}/data/ingestion/$(date +%Y-%m-%d)/candidates.jsonl
```
...
p = f"${CLAW_NEWS_DIR}/data/ingestion/{date.today().isoformat()}/candidates.jsonl"
with open(p) as f:
...
import json
from datetime import date
p = f"${CLAW_NEWS_DIR}/data/github/{date.today().isoformat()}/repos.json"
with open(p) as f:
    repos = json.load(f)
...
回放严格只读：不要通过它修复或清理生产状态，不要增加 webhook 参数，也不要提交生产 `data/`、候选原文、运行时配置或任何凭据。日常入口见 [日常巡检](../../../运维/日常巡检.md)。
