...
```bash
wc -l ${CLAW_NEWS_DIR}/data/ingestion/$(date +%Y-%m-%d)/candidates.jsonl 2>/dev/null
```
...
```bash
bash ${CLAW_NEWS_DIR}/scripts/quick-verify.sh
```
...
若需保存回放结果用于讨论，只保存脱敏后的汇总输出；不得把生产候选原文、运行时 `data/`、`feeds.yaml`、webhook 或密钥提交到仓库。常见失败与结果异常的处理见 [故障排查](../../../运维/故障排查.md)。
